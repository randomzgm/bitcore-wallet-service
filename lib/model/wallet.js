...
Wallet.prototype.getColdStakingAddresses = function() {
  var self = this,
      addresses = {};
  ...
  if (self.coldStakingSetup.staking_key.startsWith('pcs') ||
    ...
    if (!self.coldStakingSetup.spend_address) {
      self.coldStakingSetup.spend_address = self.createAddress(true, true);
    }
    ...
    const coldStakingAddress = xPub
      .derive(this.addressManager.getColdStakingAddressIndex())
      .publicKey.toAddress()
      .toString();

    addresses['staking_address'] = coldStakingAddress;
    addresses['spend_address'] = self.createAddress(true, true);
  }
