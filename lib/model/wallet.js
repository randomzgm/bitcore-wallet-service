...
Wallet.prototype.updateColdStakingAddress = function(coldStakingAddress) {
  var self = this;

  self.coldStakingAddress = coldStakingAddress;
};

Wallet.prototype.createColdStakingAddress = function() {
  $.checkState(this.isComplete());

  var self = this;

  if (!self.coldStakingAddress || self.coin !== 'part')
    return null;

  if (self.coldStakingAddress.startsWith('pcs') || self.coldStakingAddress.startsWith('tpcs')) {
    // TODO: create proper address for this
    return null;
  }

  var path = this.addressManager.getNewColdStakingAddressPath();
  log.verbose('Deriving addr:' + path);
  var address = Address.derive(self.id, this.addressType, [{xPubKey: self.coldStakingAddress}], path, this.m, this.coin, this.network, false, false, true);
