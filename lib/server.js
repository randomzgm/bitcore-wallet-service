...
WalletService.prototype.createTx = function(opts, cb) {
  ...
  function getChangeAddress(wallet, cb) {
    if (wallet.singleAddress) {
      self.storage.fetchAddresses(self.walletId, function(err, addresses) {
        ...
        return cb(null, _.head(addresses), wallet.createColdStakingAddress());
      });
    } else {
      if (opts.changeAddress) {
        self.storage.fetchAddressByWalletId(wallet.id, opts.changeAddress, function(err, address) {
          ...
          return cb(null, address, wallet.createColdStakingAddress());
        });
      } else {
        var sha256 = ( wallet.coin === 'part' && wallet.coldStakingAddress ? true : false);
        
        return cb(null, wallet.createAddress(true, sha256), wallet.createColdStakingAddress());
      }
    }
  };
  ...
  self._runLocked(cb, function(cb) {
    var txp, changeAddress, feePerKb, coldStakingAddress;
    self.getWallet({}, function(err, wallet) {
      if (err) return cb(err);
      if (!wallet.isComplete()) return cb(Errors.WALLET_NOT_COMPLETE);
      ...
      checkTxpAlreadyExists(opts.txProposalId, function(err, txp) {
        ...
        async.series([
          ...
          function(next) {
            if (opts.sendMax) return next();
            getChangeAddress(wallet, function(err, address, stakingAddress) {
              if (err) return next(err);
              changeAddress = address;
              coldStakingAddress = stakingAddress;
              next();
            });
          },
          function(next) {
...
/**
 * Update the wallets cold staking address
 * @param {Object} opts
 * @param {string} opts.coldStakingAddress - Either the cold staking bech32 address or xpub key
 */
WalletService.prototype.updateColdStakingAddress = function(opts, cb) {
  var self = this;
  ...
  self._runLocked(cb, function(cb) {
    self.getWallet({}, function(err, wallet) {
      ...
      wallet.updateColdStakingAddress(opts.coldStakingAddress);
