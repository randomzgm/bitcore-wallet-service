...
WalletService.prototype.createTx = function(opts, cb) {
  ...
  function getChangeAddress(wallet, cb) {
    // TODO: Wallets with single addresses not sha256 cannot cold stake
        ...
        async.series([
          ...
          function(next) {
            ...
            getChangeAddress(wallet, function(err, address) {
              ...
              if (opts.coldStakingAddress)
                coldStakingAddress = wallet.createColdStakingAddress(opts.coldStakingAddress);
...
WalletService.prototype._normalizeTxHistory = function(txs) {
  ...
  return _.map([].concat(txs), function(tx) {
    ...
    return {
      ...
      fees: tx.isCoinStake ? 0 : parseInt((tx.fees * 1e8).toFixed(0)),
      size: tx.size,
      time: t,
      inputs: inputs,
      outputs: outputs,
  ...
  function scanBranch(wallet, derivator, cb) {
    ...
    async.whilst(function() {
      ...
      checkActivity(wallet, address.address, function(err, activity) {
        ...
        if (_.isEmpty(addresses)) // Only increment when there is any activity on a address derived from this index
          ...
          async.eachSeries(derivators, function(derivator, next) {
            ...
            self.storage.fetchWallet(wallet.id, function(err, wallet) {
              ...
              self.storage.storeWallet(wallet, function() {
                self.storage.storeTwoStepCache(wallet.id, {}, function(err) {
                  return cb(error);
                });
              });
            })
